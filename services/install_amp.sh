...
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query "Account" --output text --profile megazone --region ap-east-1)
OIDC_PROVIDER=$(aws eks describe-cluster --name $CLUSTER_NAME --query "cluster.identity.oidc.issuer" --output text --profile megazone --region ap-east-1 | sed -e "s/^https:\/\///")
...
function getRoleArn() {
  OUTPUT=$(aws iam get-role --role-name $1 --query 'Role.Arn' --output text --profile megazone --region ap-east-1 2>&1)
...
then
  ...
  --query "Role.Arn" --output text --profile megazone --region ap-east-1)
  #
  ...
  --query 'Policy.Arn' --output text --profile megazone --region ap-east-1)
  #
  # Attach the required IAM policies to the IAM role created above
  #
  aws iam attach-role-policy \
  --role-name $SERVICE_ACCOUNT_IAM_AMP_INGEST_ROLE \
  --policy-arn $SERVICE_ACCOUNT_IAM_AMP_INGEST_ARN --profile megazone --region ap-east-1
else
